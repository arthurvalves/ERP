...
def process_nfe_xml(xml_content: str):
    ...
    try:
        # 10. IDEMPOTÊNCIA: Verifica se a NF-e já foi importada
        ...
        # 7. REGISTRO DE COMPRA (Criado antes para gerar ID de referência para itens e estoque)
        cur.execute("""
        ...
        # Cache temporário para Matching de Produtos (Reduz acessos sequenciais)
        ...
        for item in nfe_data['items']:
            ...
            # 4.2 Match por NCM + Supplier + Referência (Código Produto)
            ...
            # 4.3 Match Fuzzy pela Descrição
            if not matched_product:
        ...
        # 8. DISPARO DE EVENTOS (Desacoplado)
        try:
            from erp_backend.core.events.event_bus import emit
            emit("NFeImported", {"chave_acesso": chave, "purchase_id": purchase_id})
        except ImportError:
            pass # Fallback caso event_bus ainda não esteja instanciado em algumas etapas de ambiente local
